...
class QgsLogHandler(logging.Handler):
    ...
    def emit(self, record):
        ...
        try:
            ...
            QgsMessageLog.logMessage(record.getMessage(), 'SaVap', 0)
        except MemoryError:
            message = tr(
                'Due to memory limitations on this machine, SaVap can not '
                'handle the full log')
            print message
            QgsMessageLog.logMessage(message, 'SaVap', 0)
...
def setup_logger(logger_name, log_file=None, sentry_url=None):
    ...
    settings = QSettings()
    flag = settings.value('SaVap/useSentry', False, type=bool)
